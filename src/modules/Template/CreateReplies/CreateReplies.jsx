...
import {
  ...
  setAllReplyTemplateCategoriesEmpty,
  updateReplyTemplate,
} from "@/store/actions";
...
const CreateReplies = () => {
  ...
  const { setFieldValue, ...formik } = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (newCategoryId && newCategoryId !== body.category) {
        const updatedCategories = replyTemplateCategories.filter(
          (cat) => cat._id !== newCategoryId
        );
        dispatch({ type: "UPDATE_REPLY_TEMPLATE_CATEGORIES", payload: updatedCategories });
        ...
        dispatch(
          updateReplyTemplate({ body, _id: templateId }, () => {
            ...
            setSelectedCategoryName(
              replyTemplateCategories.find((cat) => cat._id === body.category)?.name
            );
  ...
  useLayoutEffect(() => {
    ...
    setSelectedCategory(singleTemplate?.category?._id ?? "");
    setSelectedCategoryName(singleTemplate?.category?.name ?? "");
      ...
      <div className="wrapper">
                  ...
                  <div className="top">
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedCategoryName(e?.label ?? "Select Category");
                      }}
                      options={
        ...
        <div className="bottomComplete" style={{ display: 'flex', alignItems: 'center' }}>
          <button
            type="submit"
            onClick={() => {
             dispatch(setAllReplyTemplateCategoriesEmpty()) 
              navigate(`/templates/quick-replies`)
            }}
            className="cancelBtn"
          >
