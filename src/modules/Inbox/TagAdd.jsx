          ...
          {tags?.map((tag) => (
            ...
            >
              <button style={{ backgroundColor: findIndex(userInboxItem?.tags, { _id: tag._id }) != -1 && "rgba(25, 118, 210, 0.08)" }} className="item" onClick={() => {
                ...
                ></span>
                <span style={{ display: "flex", justifyContent: "space-between", flex: 1, }} className="text"><span style={{flex:1 , overflow:"hidden" , whiteSpace: "nowrap" , textOverflow: "ellipsis"}}>{tag.name}</span>{findIndex(userInboxItem?.tags, { _id: tag._id }) != -1 && <RiIndeterminateCircleFill style={{ color: "#b9b9b9", fontSize: "20px" }} />}</span>
              </button>
            </When>
          ))}
        </Else>
