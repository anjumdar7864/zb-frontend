...
import {
  addStatusToInbox,
  addStatusToInboxList,
  changeLeadName,
  removeStatusToInbox,
  removeStatusToInboxList,
  UnAssignDripToInbox,
} from "@/store/actions";
...
const StatusUpdateMenu = ({
  ...
  const selectWrongStatus = (e, status) => {
    ...
    if (size(phoneList) == 1) {
      ...
      // setOpenModal(true);
      if (selectedUserInbox?.isVerifiedNumber || selectedUserInbox?.isVerifiedNumberPhone2 || selectedUserInbox?.isVerifiedNumberPhone3) {
        toast.error("Verified Number cannot be marked as wrong number");
      } else {
  ...
  const addStatus = (status, body) => {
    if (chatboxProps) {
      console.log("chatboxProps", chatboxProps);
      dispatch(
      ...
      console.log("chatboxProps not", chatboxProps);

      dispatch(
        addStatusToInboxList(id, body, () => {
          const { message } = statusIcons[status.name];
          if (message) {
            toast(message?.content, {
              style: {
    ...
    if (selectedId) {
      dispatch(
        removeStatusToInbox(id, body, () => {
          toast(`${userName}'s status has been removed.`, {
            style: {
      ...
      dispatch(
        removeStatusToInboxList(id, body, () => {
          toast(`${userName}'s status has been removed.`, {
            style: {
              backgroundColor: color.Info,
              color: color.White,
      ...
      if (currentStatus == status._id) {
        dispatch(
          UnAssignDripToInbox(id, () => {
            toast(`Status has been updated to "No Status".`, {
              style: {
                backgroundColor: color.Info,
                color: color.White,
                width: "380px",
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
      <div className={styles.StatusUpdateMenuStyledLeft} ref={ref}>
        <AnimatePresence mode="wait">
          {isMouseHover && !isEdit ? (
            <motion.div
              style={{
                ...
                padding: "2px 4px",
                paddingRight: "0px",
                boxShadow: "2px 2px 0px 1px rgba(26, 28, 34, 0.03)",
              }}
              className={styles.StatusUpdateMenuHoverItems}
                ...
                ?.map((status) => (
                  ...
                  >
                    <button
                      ...
                      className={
                        styles.StatusUpdateMenuHoverItemsButton +
                        (currentStatus === status._id ? " " + styles.StatusUpdateMenuStyledLeftSelected : "")
                      }
                      style={{
          ...
          {!isMouseHover || (isMouseHover && isEdit) ? (
            <motion.div
              className={styles.StatusUpdateMenuItems}
              style={{ padding: "3px 0px", gridTemplateColumns: chatboxProps ? "repeat(2, auto)" : "repeat(8, auto)" }}
              initial={{ opacity: 0 }}
              key={"two"}
              animate={{
                opacity: 1,
            ...
            >
              <When condition={!!selectedStatus}>
                <button className={styles.StatusUpdateMenuHoverItemsSelected} style={{ "--color": selectedStatus?.color }}>
              ...
              {isEdit ? (
                ...
                <h6
                  ...
                  className={` ${chatbox ? "body3Medium" : "body4Medium"} textPrimeryColor`}
                >
                  {userName}
                </h6>
              )}
        ...
        <ModalTop open={openDripWarningModal} onClose={() => { }}>
          <DripWarningModal
            ...
            deleteItemText={`${wrongNumberHeading
                ? wrongNumberHeading
                : dncNumberHeading
                  ? dncNumberHeading
                  : "DNC"
