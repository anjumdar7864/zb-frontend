...
const FolderContainer = styled.div`
  ...
  padding-bottom: 17px;
`;
const RolesAndPermissionsModal = ({ roles, userRoll, onClose, userId, allowedPermissions, setAllowedPermissions }) => {
    const [permissionData, setPermissionData] = useState();
    const [roleName, setRoleName] = useState("")
    const [roleColor, setRoleColor] = useState("")
    const [isLoading , setLoader] = useState(false)
    const [payload, setPayload] = useState({ role: '', color: '', permissions: [] });
    ...
    const handleRollName = () => {
        const data = roles.find((e) => e._id == userRoll)
    ...
    //     }
    //     console.log("log chulling for" , allowedPermissions);
        
    //     setAllowedPermissions([...allowedPermissions, `${type}_${name}`]);
    ...
    const saveHandler = () => {
        ...
        //         if(item.control == "full"){
        //             permissions = [...permissions, ...item.privilege];
        //         }else if (item?.control == "view") {
        //             const viewPrivileges = item.privilege.filter(priv => priv.startsWith("View"));
        //             permissions = [...permissions, ...viewPrivileges];
        //         }
        //     });
        // }
        // dispatch(UpdateSingleUserPermission({permissions}, userId , ()=>onClose()));

        const permissions = allowedPermissions
        dispatch(UpdateSingleUserPermission({ permissions }, userId, () => {
            toast.success("The permission has been successfully updated.")
            setLoader(false)
            onClose()
        } , (message)=>{
            toast.error(message)
    ...
    return (
        ...
        >
            ...
            >
                <p style={{ color: "#012635", fontSize: "18px", fontWeight: 600 }}>Update Role</p>
            ...
            <div style={{ height: "100%"  , display:"flex" , flexDirection:"column" , overflow:"auto" }}>
                ...
                >
                    ...
                    <RolesAndPermissionsModalTable allowedPermissions={allowedPermissions} permissionData={permissionData} handleCheckboxChange={handleCheckboxChange} />
