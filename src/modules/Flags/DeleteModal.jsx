...
export default function DeleteModal({ open, setOpen, curruntNumber, selectedNumber, tenantId, refreshDetails, originalNumber , noMessageSent }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const { data, isError, message, sessionExpired } = await commonAPICall(
                ...
                `${ENDPOINTS.DELETE_NUMBER}${curruntNumber ? `?phone=${curruntNumber}` : ""}${originalNumber ? `&originalNumber=${originalNumber}` : ""}`,
            ...
            setLoading(false)
            setOpen(false)
            refreshDetails(selectedNumber, tenantId , noMessageSent)
        } catch (error) {
            setLoading(false)
            console.log(error);
        }
            ...
            >
                <Box sx={style}>
                    <div style={{ display: "flex", justifyContent: "space-between", height: "58px", alignItems: "center", padding: "0px 16px", borderBottom: "solid 1px #F7F7F7" }}>
                        <div style={{ color: "#012635", fontSize: "18px", fontWeight: 600 }}>XYZ Solution</div> <div><IoMdClose style={{ cursor: "pointer" }} onClick={handleClose} size={24} /></div>
                    </div>
                    <div style={{paddingTop: "16px"}}>
                        ...
                        <div style={{ fontSize: "16px", color: "#012635", fontWeight: 400, padding: "16px" }}>
                            This will permanently delete this out outbound number <span style={{ fontWeight: 600 }}>1{curruntNumber}</span> . Deleting this outbound number will delete all associated data, such as are code, connected customer numbers etc.
                            <br />  <br />To confirm, please enter your password.
                        </div>
